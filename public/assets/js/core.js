...
document.querySelector(".new-link").addEventListener("click", function(){
  if(post_form.style.display == "block"){
    ...
    if(window.ga){
      window.ga('send', 'event', "Links", "Button click", "New link form expanded", window.location.pathname.replace("/user/", ""));
    }
  }
});
...
post_form.addEventListener("submit", function(e){
  ...
  if(window.ga){
    ga('send', 'event', "Links", "Button click", "New link form submitted", window.location.pathname.replace("/user/", ""));
  }
